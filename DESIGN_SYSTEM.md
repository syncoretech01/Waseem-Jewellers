...
That figure appears on the `hairline` Button variant and the collection index rows; `PieceLink` runs the same span along its top edge. `ChoiceRow` and the menu overlay use the same draw driven by selection rather than hover. `<Hairline />` in `src/components/ui/primitives.tsx` is the static full-width version.
...
3. mirrors that section's theme onto the chrome's `[data-chrome]` wrappers (`data-theme`) and sets the active section in `siteStore`; `<html>` keeps a static `data-theme="dark"`.
...
Chapter themes as shipped: hero, craft, diamond, collections, bespoke, the collection opening and the product gallery are `dark`; the window, the bangle, gold, bridal, men and kids, heritage and the collection intro/index/details are `ivory`. Two sections take their theme from a prop or from data: the "worn together" rail is `ivory` on the product route and `dark` on the collection route, and the collection story chapters carry the theme set on each chapter in `src/data/collections.ts`. `Dialog` opens its panel with its own `data-theme` (the consultation modal is an `ivory` island).
...
| `--content-max` | `1600px` | `.wj-content` — the measured width every chapter's content sits within, centred inside the gutter; `.wj-row` is the same cap for a padded row that carries its own `px-gutter` (the refine bar). The home chapters, the heritage chapter and the department pages (masthead, intro, refine bar, index, closing) carry it. At 1600px it has no effect below a 1744px viewport. |
...
| `--chapter-y` | `clamp(4.5rem, 12svh, 8rem)` | a chapter's padding below, and the space between two department rows from md |
| `--chapter-pt` | `max(var(--chapter-y), var(--chapter-top))` | a chapter's padding **above**: its own space, never less than the clearance beneath the fixed header (`--chapter-top` = nav + safe-area + 1.25rem, `globals.css`). Every unpinned chapter and every department page opening is `pt-[var(--chapter-pt)] pb-[var(--chapter-y)]`, so a chapter scrolled to its top on a short phone keeps its first line out from behind the nav; the pinned chapters and the worlds set their first line at `--chapter-top` directly |
...
| `--lift` | `6svh` | the one editorial offset a composition may use, at most once per cluster — currently unused; nothing is offset with a negative margin, except a phone's rail (below) |
| `--rail-fade` | `2.75rem` | the width of the fade a phone's rail draws over an edge that still has more to scroll |
...
**Rails.** A rail is `Rail` (`src/components/ui/Rail.tsx`, `.wj-rail` + `.wj-rail-wrap`): a `ul` that scrolls sideways on a phone and stands as a column from md. On a phone it runs edge to edge — negative `margin-inline` of one gutter, the same gutter back as `padding-inline` and `scroll-padding-inline` — so the first item lines up with the chapter's left edge, the last has the gutter after it, and a word is never cut at the page's edge; momentum and snapping (`x proximity`) are the browser's, the scrollbar is hidden. The wrapper measures the scroll and writes `data-start` / `data-end`; its `::before` / `::after` draw a fade of `--rail-fade` in the page's own `--bg` over whichever edge still has more to scroll, and none at an end that has been reached, so the first and last items always stand whole. Every item is a touch target at least 44px tall (`.wj-rail-item` is 48px; the department's kinds are `min-h-11`). Two rails use it: the window's kinds (display face, hairline beneath the active one) and a department page's kinds (eyebrow face with counts). The concierge result tray keeps its own `no-scrollbar` row with `scrollPaddingLeft: var(--spacing-gutter)`.

**Touch targets.** A text control that must keep its line but answer a finger — a facet, a *Map* link, *Show 24 more*, a removable facet in the refine line — carries `.wj-hit`, whose `::before` draws a hit area 10px above and below and 6px either side without laying anything out. Two-column phone rows (`.wj-kinds`) and rail items are sized 44–48px tall outright.

**The phone.** A chapter on a phone is designed, not stacked: the jewellery is the second thing on the screen (the window's plate after a three-word title; a department's tray after its title and one line), body copy that only explains is left to md (`hidden md:block`), the kinds of a department are two columns of hairline rows rather than seven, two department rows in one chapter are a rule and 3.5rem apart, and a phone display title is 2–2.25rem (`clamp` minimums), never a single word per line. Reveals are kept for a chapter's title and its tray or hero object; the line, the kinds, the doors and the practical facts are simply there.
...
The block does structural work, not just animation suppression. The pinned chapters (the bangle, the goldwork, the light, bespoke) build no pin under reduced motion and present their desktop stage as a composed still — the piece whole, one stage word — so nothing is unreachable and nothing is stacked twice. The concierge orb core and the invitation ring stop animating.
