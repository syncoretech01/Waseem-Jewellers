// Renders the craft chapter's ring at each beat of its sequence, from the same scene the page
// draws, into the frames the chapter shows where the live object is not mounted: every phone,
// a device without WebGL, a context lost twice. The chapter's `CRAFT_FRAMES` (craftProgress.ts)
// is the one list of beats; this script reads it, scrolls the pinned chapter to each progress,
// waits for the scrub and the damped object to settle, captures the canvas, then cuts every
// frame to ONE crop box (the union of the object's extent across all frames, with a margin)
// so the object never jumps between frames. Run against a dev server (?tier=HIGH is honoured
// there) — the scene is rendered through SwiftShader, which is slow and exact.
//   node scripts/assets/craft-frames.mjs http://localhost:3300
//   node scripts/assets/craft-frames.mjs http://localhost:3300 --at=.06,.21,.39 --out=.cache/s22/craft/probe
...
// 90 on the ink and a fifth smaller — the 1080w set of 23 frames stays near a megabyte
const ALPHA_QUALITY = Number(val('alpha') || 70);
const MARGIN = Number(val('margin') || 0.05);
// the canvas is square, so the camera frames the object as it does on a desktop stage; wide
// enough that the crop — the object fills a little over half of it — carries 1600 real pixels
const SIDE = Number(val('side') || 2600);
fs.mkdirSync(OUT, { recursive: true });

/** The beats, from the one list the chapter reads. */
function readFrames() {
  const src = fs.readFileSync(path.resolve('src/components/home/chapters/craftProgress.ts'), 'utf8');
  const block = src.match(/CRAFT_FRAMES[^=]*=\s*\[([\s\S]*?)\];/)?.[1];
  if (!block) throw new Error('CRAFT_FRAMES not found in craftProgress.ts');
  return [...block.matchAll(/key:\s*'([a-z0-9-]+)'[^}]*?\bp:\s*([\d.]+)/g)].map((m, i) => ({ i, key: m[1], p: Number(m[2]) }));
}
const FRAMES = val('at') ? val('at').split(',').map((p, i) => ({ i, key: `p${p.replace(/^0?\./, '')}`, p: Number(p) })) : readFrames();
console.log('frames', FRAMES.map((f) => `${f.i}:${f.key}@${f.p}`).join(' '));
...
for (const f of FRAMES) {
  ...
  const settled = still >= 2;
  const y = await page.evaluate(() => Math.round(window.scrollY));
  const progress = (y - top) / pin;
  ...
  captures.push({ ...f, png: prev, box, progress });
  if (val('out')) fs.writeFileSync(path.join(OUT, `raw-f${f.i}.png`), prev);
  console.log(`f${f.i} ${f.key} p=${f.p} (scrolled ${progress.toFixed(3)}) ${settled ? 'settled' : 'NOT settled'} box ${box.width}x${box.height}@${box.left},${box.top} ${((Date.now() - t0) / 1000).toFixed(0)}s`);
}
await browser.close();

// one crop box for every frame: the union of the object's extents, with a margin
...
const crop = { left: Math.max(0, u.l - m), top: Math.max(0, u.t - m), right: Math.min(SIDE, u.r + m), bottom: Math.min(SIDE, u.b + m) };
// even numbers keep the resized widths' rounding identical across frames
const W = (crop.right - crop.left) & ~1;
const H = (crop.bottom - crop.top) & ~1;
console.log(`union ${u.r - u.l}x${u.b - u.t}@${u.l},${u.t}; crop ${W}x${H}@${crop.left},${crop.top}; aspect ${(W / H).toFixed(4)}`);
...
for (const c of captures) {
  ...
  for (const w of WIDTHS) {
    const file = path.join(OUT, `ring-f${c.i}-${w}w.webp`);
...
for (const w of WIDTHS) {
  ...
  console.log(`${w}w set: ${set.length} frames, ${(set.reduce((a, b) => a + b.bytes, 0) / 1024).toFixed(0)} KB total, ${set.map((x) => (x.bytes / 1024).toFixed(0)).join('/')} KB`);
}
// the record of the run (crop box, beats): beside a probe set, else in .cache — never shipped
const manifest = val('out') ? path.join(OUT, 'frames.json') : path.resolve('.cache/craft-frames.json');
fs.mkdirSync(path.dirname(manifest), { recursive: true });
fs.writeFileSync(
  manifest,
  JSON.stringify({ width: W, height: H, aspect: +(W / H).toFixed(4), crop, quality: QUALITY, alphaQuality: ALPHA_QUALITY, frames: captures.map((c) => ({ i: c.i, key: c.key, p: c.p, box: c.box })) }, null, 1),
);
console.log('frames →', OUT, `(${W}x${H}, aspect ${(W / H).toFixed(4)}); set CRAFT_FRAME_SIZE in craftProgress.ts and --craft-frame-aspect in craft.css if they changed`);
