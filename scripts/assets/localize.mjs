...
const videoMap = Object.fromEntries(
  videos.map((v) => [
    v.id,
    {
      ...
      src1280Av1: v.files['1280'].srcAv1,
      src720Av1: v.files['720'].srcAv1,
      srcPortraitAv1: v.files.portrait.srcAv1,
      poster: v.poster,
...
lines.push('## Videos', '', 'Each tier is encoded twice; a visit downloads one tier of one codec. AV1 is offered first and taken by every current browser; the h264 sibling is the fallback.', '', '| id | use | duration | 1280 h264 / av1 | 720 h264 / av1 | portrait h264 / av1 | poster |', '|---|---|---|---|---|---|---|');
for (const v of videos) {
  const pair = (f) => (f.bytesAv1 ? `${fmtBytes(f.bytes)} / ${fmtBytes(f.bytesAv1)}` : fmtBytes(f.bytes));
  lines.push(`| ${v.id} | ${v.label} | ${v.duration.toFixed(1)}s | ${pair(v.files['1280'])} | ${pair(v.files['720'])} | ${pair(v.files.portrait)} | ${v.poster} |`);
}
