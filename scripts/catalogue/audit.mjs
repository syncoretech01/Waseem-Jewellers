...
**Synced** ${new Date().toISOString().slice(0, 10)} · **fetched** ${products.length + Object.keys(excluded).length} · **imported** ${products.length} · **listable** ${stats.listable} · **withheld** ${stats.withheld}
...
${['gold', 'diamond', 'bridal', 'men', 'kids']
  .map((d) => {
    ...
    return `| ${d} | ${all.length} | ${live} | ${stats.flagshipByDept?.[d] ?? 0} |`;
  })
  .join('\n')}
...
**This is what makes Bridal read thin: ${products.filter((p) => p.departments.includes('bridal') && p.listable).length} of ${products.filter((p) => p.departments.includes('bridal')).length} bridal pieces are listable.**
...
console.log(`\n${products.length} imported, ${stats.listable} listable, ${stats.withheld} withheld`);
