...
// ── the facet index the client is allowed to ship ───────────────────────────
// The full catalogue is ~1 MB and belongs to the server and the build. This is what a facet
// UI actually needs, and nothing more.
const slim = emitted
  .filter((p) => p.listable)
  .map((p) => ({
    s: p.slug,
...
await writeJson(path.join(GENERATED, 'catalogue.slim.json'), { syncedAt, count: slim.length, products: slim });
...
const slimLoader = `/* Generated by scripts/catalogue — do not edit. Run \`npm run catalogue\`. */
import data from './catalogue.slim.json';

/**
 * What a facet UI needs and nothing more: one row per listable piece, short keys, about
 * ${Math.round(JSON.stringify(slim).length / 1024)} kB raw. This is the only catalogue data the browser is ever sent, and it is
...
`;
await fs.writeFile(path.join(GENERATED, 'catalogue.slim.ts'), slimLoader, 'utf8');
...
console.log(`slim index     ${slim.length} listable    ${size({ products: slim })} kB`);
...
console.log(`\n→ ${rel(GENERATED)}/catalogue{,.slim}.{json,ts}, taxonomy.json`);
