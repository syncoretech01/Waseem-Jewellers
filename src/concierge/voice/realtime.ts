...
const TRACE_MAX = 400;
/**
 * Function-call ids are only a replay guard for late provider events. Keeping every id for
 * a multi-hour open session is needless retained state; this window covers forty maximum
 * four-tool turns, which is far beyond an in-flight or late event on an ordered channel.
 */
const ANSWERED_CALLS_MAX = 160;
...
const RESPONSE_WATCHDOG_MS = 28_000;
...
// Retained for transcript-script diagnostics when the provider supplies that metadata.
...
export class RealtimeVoiceAdapter implements VoiceAdapter {
  ...
  private responseSettleTimer: number | null = null;
  ...
  /** Provider response IDs keep a cancelled response's late events away from the next reply. */
  private activeResponseId: string | null = null;
  private speaking = false;
  private turnHadTool = false;
  private appointmentReviewAwaitingConfirmation = false;
  /** The review's visitor turn; submission needs a later visitor turn, not a same-response shortcut. */
  private appointmentReviewUtteranceId: string | null = null;
  ...
  private supersede(reason: string) {
    this.epoch += 1;
    this.pending.clear();
    // Realtime requires cancellation before clearing WebRTC's buffered audio.
    if (this.responseOpen) {
      this.note('response.cancel', reason);
      this.send({ type: 'response.cancel' });
    }
    // Audio may still be playing after response.done, in which case clearing alone is correct.
    if (this.responseOpen || this.speaking) {
      this.note('audio.clear', reason);
  ...
  private async connect(h: VoiceHandlers) {
    ...
    try {
      ...
      pc.ontrack = (e) => {
        ...
        stream.getAudioTracks().forEach((remoteTrack) => {
          this.note('audio.track', remoteTrack.readyState);
          remoteTrack.onended = () => this.note('audio.track.ended');
        });
      };
      ...
      pc.onconnectionstatechange = () => {
        this.note('peer.state', pc.connectionState);
  ...
  private teardown(reason: string) {
    ...
    this.appointmentReviewAwaitingConfirmation = false;
    this.appointmentReviewUtteranceId = null;
  ...
  private clearResponseTimers() {
    if (this.responseTimer) window.clearTimeout(this.responseTimer);
    if (this.responseSettleTimer) window.clearTimeout(this.responseSettleTimer);
  ...
  private armResponseWatchdog() {
    ...
    this.responseTimer = window.setTimeout(() => {
      this.responseTimer = null;
      if (!this.live || !this.responseOpen || this.running > 0) return;
      this.note('response.watchdog', 'no completion');
      this.send({ type: 'response.cancel' });
      this.send({ type: 'output_audio_buffer.clear' });
      this.responseOpen = false;
      this.activeResponseId = null;
      ...
      if (turnId) this.runtime?.emit({ type: 'turn.error', turnId, message: 'response timeout', recoverable: true });
      this.turnId = null;
      this.replyText = '';
      this.outstanding = false;
      this.armIdle();
  ...
  private scheduleResponseSettlement() {
    if (this.responseSettleTimer) window.clearTimeout(this.responseSettleTimer);
    this.responseSettleTimer = window.setTimeout(() => {
      this.responseSettleTimer = null;
      if (this.responseOpen) return;
  ...
  /** `response.done` and audio-buffer events can arrive after a barge-in has begun a new reply. */
  private isCurrentResponse(e: RealtimeEvent): boolean {
    const response = (e.response ?? {}) as Record<string, unknown>;
    const id = typeof e.response_id === 'string' ? e.response_id : typeof response.id === 'string' ? response.id : '';
    if (id && this.activeResponseId && id !== this.activeResponseId) {
      this.note('response.stale', `${e.type}:${id}`);
      return false;
    }
    return true;
  }
  ...
  private onEvent(e: RealtimeEvent) {
    ...
    switch (e.type) {
      ...
      case 'input_audio_buffer.speech_started': {
        ...
        this.interim = '';
        this.note('interruption', 'user speech');
        break;
      }
      ...
      case 'conversation.item.input_audio_transcription.failed': {
        ...
        this.interim = '';
        h.onFinal(partial);
        if (this.utteranceId) rt.emit({ type: 'voice.utterance', id: this.utteranceId, text: partial || 'Speech was not captured.', final: true, lost: !partial });
        break;
      }

      case 'response.created': {
        const response = (e.response ?? {}) as Record<string, unknown>;
        const responseId = typeof e.response_id === 'string' ? e.response_id : typeof response.id === 'string' ? response.id : '';
      ...
      case 'response.output_text.delta': {
        if (!this.isCurrentResponse(e)) break;
        this.note('response.output_audio.delta', String(e.delta ?? '').length.toString());
        this.armResponseWatchdog();
      ...
      case 'response.done': {
        ...
        if (response.status === 'cancelled') {
          ...
          this.note('response.cancelled', String(details.reason ?? 'provider'));
        }
        if (response.status === 'incomplete') {
          const details = (response.status_details ?? {}) as Record<string, unknown>;
          this.note('response.incomplete', String(details.reason ?? 'provider'));
        }
        ...
        this.scheduleResponseSettlement();
        break;
      }
  ...
  private queueCall(callId: string, rawName: string, args: string) {
    ...
    if (!this.responseOpen) this.scheduleResponseSettlement();
  }

  /** Remember a completed call only for the bounded late-event replay window. */
  private markAnswered(callId: string) {
    this.answered.add(callId);
    while (this.answered.size > ANSWERED_CALLS_MAX) this.answered.delete(this.answered.values().next().value!);
  }
  ...
  private async drainCalls() {
    ...
    for (const call of calls) {
      ...
      if (call.name === 'fillAppointment' || call.name === 'cancelAppointment') {
        this.appointmentReviewAwaitingConfirmation = false;
        this.appointmentReviewUtteranceId = null;
      }
      if (call.name === 'submitAppointment' && (!this.appointmentReviewAwaitingConfirmation || !this.utteranceId || this.utteranceId === this.appointmentReviewUtteranceId)) {
        this.note('appointment.submit.blocked', this.appointmentReviewAwaitingConfirmation ? 'later confirmation required' : 'review required');
        this.send({ type: 'conversation.item.create', item: { type: 'function_call_output', call_id: call.callId, output: JSON.stringify({ error: 'CONFIRMATION_REQUIRED', message: 'Review the complete appointment first, then wait for a later explicit visitor confirmation before submitting.' }) } });
        answered = true;
        this.running -= 1;
        continue;
      }
      try {
        ...
        if (call.name === 'reviewAppointment') {
          const result = outcome.result as { missing?: unknown } | undefined;
          this.appointmentReviewAwaitingConfirmation = Array.isArray(result?.missing) && result.missing.length === 0;
          this.appointmentReviewUtteranceId = this.appointmentReviewAwaitingConfirmation ? this.utteranceId : null;
          this.note('appointment.review', this.appointmentReviewAwaitingConfirmation ? 'awaiting confirmation' : 'incomplete');
        }
        if (call.name === 'submitAppointment') {
          this.appointmentReviewAwaitingConfirmation = false;
          this.appointmentReviewUtteranceId = null;
        }
        answered = true;
...
/** Controller-side milestones share the adapter trace used by the local QA harness. */
