...
/** the one plate spans seven of twelve columns; on a phone it is the width of the page */
const PLATE_SIZES = '(min-width: 1744px) 920px, (min-width: 768px) 54vw, 92vw';

/** What the phone says where the desktop's long title and line would take the screen the jewellery needs. */
const PHONE = {
  /** The short title over the plate. The eyebrow above it already says The Collection. */
  title: 'Explore by kind.',
} as const;
 ...
 *
 * Two grid items: the plate, and the caption beneath it. On a phone the rail stands between
 * them (the grid's `order`), so a thumb reads plate, kinds, then the piece and its door.
 */
function KindPlate({ kinds, active, lit }: { kinds: ShowcaseCategory[]; active: ShowcaseCategory; lit: boolean }) {
  ...
  const tag = tagOf(active.hero);
  return (
    <>
      <div ref={ref} className="order-1 md:order-none md:col-span-7 md:row-start-1" data-kind={active.category} data-lit={lit ? '1' : '0'} data-rise>
      ...
      {/* beneath the plate, at the caption's offset: the kind, named large, and the piece shown, named as a piece — each a door.
          On a phone the rail has already named the kind, so the piece leads and the kind's door is the bracket beneath it. */}
      <div className="wj-caption order-3 mt-6 gap-5 md:order-none md:col-span-7 md:col-start-1 md:row-start-2 md:mt-[var(--caption-gap)] md:flex-row md:items-end md:justify-between md:gap-6">
        <TransitionLink key={`kind-${active.category}`} href={active.href} className="stage-word group/door hidden items-baseline gap-4 md:flex" data-cursor="explore">
        ...
        <TransitionLink key={`piece-${active.hero.s}`} href={`/jewellery/${active.hero.s}`} className="stage-word group/piece flex flex-col gap-1 md:items-end md:text-right" data-cursor="view">
          <span className="wj-caption-name" style={{ '--card-name': '1.1875rem', '--card-opsz': 18 } as CSSProperties}>
            {active.hero.t}
          </span>
          {tag && <span className="wj-caption-tag">{tag}</span>}
          <span className="micro mt-1 text-ink/70 md:hidden">{COPY.window.view}</span>
        </TransitionLink>
        <div key={`door-${active.category}`} className="stage-word md:hidden">
          <Button variant="bracket" href={active.href} cursor="explore">
            {COPY.window.explore} {active.label}
          </Button>
        </div>
      </div>
    </>
 ...
 * the plate shows, and choosing one opens it. On a phone the jewellery comes first: a short
 * title, the plate, the kinds as a row beneath it that scrolls edge to edge, then the piece
 * and the door — a tap chooses the kind before a second tap opens it. No counts: a window
 * says what is made, not how many are in the back.
 */
export function Ch02Kinds({ kinds }: { kinds: ShowcaseCategory[] }) {
  ...
  return (
    <section ref={ref} id="ch02-kinds" data-theme="ivory" className="relative bg-ivory px-gutter pt-[var(--chapter-pt)] pb-[var(--chapter-y)] text-ink" aria-labelledby="kinds-title">
      <div ref={scope} className="wj-content">
        <div className="wj-grid md:items-end">
          <div className="flex flex-col gap-3 md:col-span-7 md:gap-4">
            <Eyebrow className="text-ink/60">{COPY.window.eyebrow}</Eyebrow>
            {/* the title: the kinds named, from md; on a phone three words, so the plate is the second thing on the screen. One heading, the
                phone's words and the desktop's each shown at their own widths, so the chapter is named once either way. */}
            <h2 id="kinds-title" className="display max-w-[13em] text-[clamp(2.25rem,4.4vw,4.75rem)] leading-[1.02] text-ink [text-wrap:balance]">
              <span data-split className="hidden opacity-0 md:block">
                {COPY.window.title}
              </span>
              <span className="md:hidden">{PHONE.title}</span>
            </h2>
          </div>
          <p className="hidden max-w-[30em] text-[0.9375rem] leading-relaxed text-ink/70 [text-wrap:pretty] md:col-span-4 md:col-start-9 md:block md:pb-2">{COPY.window.line}</p>
        </div>
        ...
        <div className="wj-grid mt-8 gap-y-0 md:mt-[var(--block-y)] md:items-stretch">
          ...
          <nav className="order-2 mt-5 flex flex-col md:order-none md:col-span-4 md:col-start-9 md:row-start-1 md:mt-0" aria-label={COPY.window.kinds}>
            {/* on a phone the rail is a row: the chosen kind is brought into it, and only the row moves — never the page */}
            <Rail active={active.category} className="md:flex md:flex-1 md:flex-col" listClassName="md:flex-1" data-kinds>
              {rail.map((kind) => {
                const on = kind.category === active.category;
                return (
