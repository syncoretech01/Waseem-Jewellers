...
**The homepage**, fifteen chapters in one continuous scroll, in the order a shop is walked — five of them signature jewellery experiences, each a chapter of its own with a beginning, one piece as protagonist, a progression, a closing state and a door:
...
| Craft · **1** | Keep scrolling: an emerald-cut stone condenses out of the dark, then stone, setting, metal and hand finishing separate beneath fixed labels, closing on CRAFTED / TO ENDURE. The object is a study — labelled as one beside the eyebrow: an emerald cut and set in gold to show how a ring is made, not a listed piece. It renders on every tier; without WebGL, or under reduced motion, a still of the same object stands in. |
| The Study · **2** | Its own pinned chapter: a pencil drawing traced from the Lavender Halo Ring's own photograph develops into the photograph — tone first, then colour — and the ring turns to the second angle Waseem shot. One stage word at a time at the right; the ring is the door beneath. |
...
| Goldwork · **3** | From the work to the piece: the camera opens on the pattern of one collar panel of the Satlada Haar filling the frame, travels down the haar to its relief and the medallion's edge, and pulls back until the whole piece is in view. On the whole piece four places become live — rest on one and the camera drifts closer. |
| Diamond | The other tray, on ink. |
| One suite, one light · **4** | Its own pinned chapter: the room goes dark and a single light finds the chandelier earring, the necklace and the pendant of the Sapphire Pendant Suite in turn, then the whole suite is lit again. |
...
| 1952 | Since 1952, Lahore, crafted across generations: the facade arriving in colour, the three showrooms — Liberty Market, MM Alam Road, DHA — with addresses, map links, hours and the telephone, the lockup, an appointment. |
| Bespoke · **5** | One pair of earrings read at the joints: the photograph is cut into crown, bell and tassel, drawn apart by nearly a tenth of the frame, held while each is named — one stage word at a time, in one node — and closed again into the pair. Personal craft · Precision · One-to-one design · Finish, said once; the bespoke door is open from the first frame. |
